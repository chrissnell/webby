...
deployable_branches=( [master]=us-east-1-prod.kube.webflow.services [dev]=us-east-1-stage.kube.webflow.services)
...
if [ -r kube-deploy/${DEPLOY_CONTEXT}/deployment.yaml ]; then
   if ! grep -q "%VERSION%" kube-deploy/${DEPLOY_CONTEXT}/deployment.yaml; then
      ...
      echo "See the deployment.yaml in the webflow-ssl-terminator repo for an example."
      exit 1
   fi
...
if [ -r kube-deploy/${DEPLOY_CONTEXT}/service.yaml ]; then
   echo "Applying service.yaml ..."
   ${KUBECTL} apply -f kube-deploy/${DEPLOY_CONTEXT}/service.yaml

   if [ $? -ne 0 ]; then
       echo "There was an error applying service.yaml."
       exit 1
   fi

   echo "Succesfully applied service.yaml."
fi


if [ -r kube-deploy/${DEPLOY_CONTEXT}/hpa.yaml ]; then
   echo "Applying hpa.yaml ..."
   ${KUBECTL} apply -f kube-deploy/${DEPLOY_CONTEXT}/hpa.yaml

   if [ $? -ne 0 ]; then
       echo "There was an error applying hpa.yaml."
       exit 1
   fi

   echo "Succesfully applied hpa.yaml."
fi
